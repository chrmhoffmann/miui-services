...
public class MiuiConfigCaptivePortal {
    ...
    public static final URL getCaptivePortalServer(Context context, String url) {
        String server = getCaptivePortalServer(null);
        if (server == null && (url == null || !url.startsWith("http"))) {
            ...
            if (server == null) {
                try {
                    ...
                    server = (String) dsFiled.get(null);
        ...
        try {
            return server != null ? new URL("http", server, "/generate_204") : new URL(url);
    ...
    private static final boolean isInCnRegion() {
        ...
        if (Build.checkRegion("CN")) {
            return !isExistIccCard || TextUtils.isEmpty(networkOperator);
        }
        return false;
    }
