...
public class VelocityMonitor {
    ...
    public float getVelocity(int idx) {
        ...
        return ((this.mHistory.size() <= 0 || Math.abs(now - this.mHistory.getLast().timeStamp) <= TIME_THRESHOLD) && (fArr = this.mVelocity) != null && fArr.length > idx) ? fArr[idx] : MiuiFreeformPinManagerService.EDGE_AREA;
    }
    ...
    private float calVelocity(int idx, MoveRecord lastRecord, MoveRecord lastRecord1) {
        ...
        return velocity == Float.MAX_VALUE ? MiuiFreeformPinManagerService.EDGE_AREA : velocity;
    }
