...
class ConnectivityServiceInjector {
    ...
    static void showLogin(Context context, Intent intent, String ssid) {
        ...
        URL urlServer = MiuiConfigCaptivePortal.getCaptivePortalServer(context, intent.getDataString());
        if (urlServer == null) {
            Slog.e(TAG, "showLogin : URL of CaptivePortalServer is NULL");
            return;
        }
        loginIntent.setData(Uri.parse(urlServer.toString()));
    ...
    static final Collection<InetAddress> addDnsIfNeeded(Collection<InetAddress> dnses) {
        if (dnses != null && dnses.size() <= 2) {
            ...
            if (hasIpV6) {
                try {
                    InetAddress inetAddress = NetworkUtils.numericToInetAddress(dnsv6);
                    dnses.add(inetAddress);
                } catch (IllegalArgumentException e) {
                    Slog.e(TAG, "Error setting dns using " + dns + " or " + dnsv6);
                }
            }
            if (hasIpV4) {
                InetAddress inetAddress2 = NetworkUtils.numericToInetAddress(dns);
                dnses.add(inetAddress2);
            }
        }
        return dnses;
    }
